<script lang="ts">
	...
	function isParticleActive(p: Particle) {
		return p.x < yearProgress + p.jitter;
	}
	...
	const totalProgressDots = 80;

	const activeDotsCount = (yearProgress / 100) * totalProgressDots;

	function getProgressDotOpacity(index: number) {
		if (index < activeDotsCount - 1) return 1;
		if (index > activeDotsCount) return 0.2;
		const decimal = activeDotsCount % 1;
